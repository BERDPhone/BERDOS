...
// TX buffer.
static uint32_t SM_FLEVEL_FREE_5_MASK = 0x000c << (SM * 8);

// Masks for the FDEBUG register.
static uint32_t SM_STALL_MASK = 1u << (PIO_FDEBUG_TXSTALL_LSB + SM);
static uint32_t SM_OVERRUN_MASK = 1u << (PIO_FDEBUG_TXOVER_LSB + SM);
